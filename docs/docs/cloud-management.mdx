...
This tutorial introduces the cloud resource management schema, which provides a vendor-agnostic way to model cloud infrastructure across AWS, GCP, and Azure. You'll load sample cloud data and explore how Infrahub can serve as a unified inventory for multi-cloud environments.
...
If you prefer to load the data manually or into an existing branch:

<Tabs>
  <TabItem value="new-branch" label="New Branch">

```bash
# Create a new branch
uv run infrahubctl branch create my-cloud-branch

# Load schemas
uv run infrahubctl schema load schemas/ --branch my-cloud-branch

# Load cloud objects
uv run infrahubctl object load objects/cloud/ --branch my-cloud-branch
```

  </TabItem>
  <TabItem value="existing-branch" label="Existing Branch">

If the cloud schema is already loaded on your branch:

```bash
# Load cloud objects to an existing branch
uv run infrahubctl object load objects/cloud/ --branch your-branch-name
```
...
The cloud schema can be extended to include additional cloud services:
...
To extend the schema, create additional node definitions in `schemas/extensions/cloud/` following the existing patterns.

## Next steps

After exploring cloud resources, you can:

- **Create branches** to model planned cloud changes
- **Use proposed changes** to review infrastructure modifications before applying
- **Integrate with cloud APIs** to sync actual cloud state into Infrahub
- **Build transforms** to generate cloud infrastructure-as-code (Terraform, CloudFormation, etc.)
