...
class TestDCWorkflow(TestInfrahubDockerWithClient):
    ...
    @pytest.mark.order(4)
    @pytest.mark.dependency(name="security_data", depends=["bootstrap_data"])
    def test_04_load_security_data(self, client_main: InfrahubClientSync) -> None:
        ...
        assert load_security.returncode == 0, (
            f"Security data load failed.\n"
            f"  Return code: {load_security.returncode}\n"
            f"  stdout: {load_security.stdout}\n"
            f"  stderr: {load_security.stderr}"
        )
    ...
    async def test_08_verify_dc_created(
        ...
        assert dc, (
            f"dc-arista topology not found.\n"
            f"  Branch: {default_branch}\n"
    ...
    async def test_09_run_generator(
        ...
        try:
            ...
            raise AssertionError(
                f"Generator definition 'create_dc' not available after waiting.\n"
        ...
        assert dc, (
            f"dc-arista topology not found before running generator.\n"
            f"  Branch: {default_branch}\n"
    ...
    async def test_14_verify_merge_to_main(
        ...
        # If dc-arista not found, gather diagnostic information
        if not dc_main:
        ...
        assert dc_main, (
            f"dc-arista not found in main branch after merge.\n"
        ...
        assert devices_main, (
            ...
            f"  dc-arista was found, but no DcimDevice objects exist.\n"
            f"  Source branch: {default_branch}"
        )
